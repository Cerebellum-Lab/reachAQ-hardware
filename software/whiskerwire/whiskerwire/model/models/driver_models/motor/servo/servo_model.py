...
class ServoModel(MotorModel):
    ...
    def min_position(self, value: float):
        if not self.is_valid_position(value):
            raise ValueError(f"Invalid Min Position <{value}>: must be on the interval [{self.MIN_POSITION},{self.MAX_POSITION}]")
    ...
    def max_position(self, value: float):
        if not self.is_valid_position(value):
            raise ValueError(f"Invalid Max Position <{value}>: must be on the interval [{self.MIN_POSITION},{self.MAX_POSITION}]")
    ...
    def min_pwm_duration_us(self, value: float):
        if not self.is_valid_pwm_duration_us(value):
            raise ValueError(f"Invalid Min PWM Duration <{value}>: must be on the interval [{self.MIN_PWM_DURATION},{self.MAX_PWM_DURATION}]")
    ...
    def max_pwm_duration_us(self, value: float):
        if not self.is_valid_pwm_duration_us(value):
            raise ValueError(f"Invalid Max PWM Duration <{value}>: must be on the interval [{self.MIN_PWM_DURATION},{self.MAX_PWM_DURATION}]")

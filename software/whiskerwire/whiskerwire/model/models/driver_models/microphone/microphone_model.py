...
class MicrophoneModel:
    ...
    def update_from_message(self, msg):
        raise NotImplementedError("update_from_message has not been implemented for the MicrophoneModel class")
    
    def register_on_fft_data_change(self, callback: callable):
        if self._on_fft_data_change is not None:
            raise RuntimeError("A callback has already been registered for on_fft_data_change")

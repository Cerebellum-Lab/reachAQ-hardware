...
class Model:
    ...
    def process_message(self, msg: JerryCANMsg):
        ...
        logger.debug(f"Received CAN Message: DeviceType={str(device_type)}, Address={can_address}, Type={msg.type}")
        ...
        if dst_id in self.modules:
            ...
            if device_type == PelletModuleModel.DEVICE_TYPE:
                logger.info(f"New Pellet Module detected with CAN address: {can_address}")
                self.modules.update({dst_id: PelletModuleModel(can_address)})
            elif device_type == MagnetModuleModel.DEVICE_TYPE:
                logger.info(f"New Magnet Module detected with CAN address: {can_address}")

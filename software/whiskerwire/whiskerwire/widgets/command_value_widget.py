...
class CommandValueWidget(Widget):
    ...
    def __init__(self, widget_name: str, placeholders: list[str], orientation=Orientation.HORIZONTAL, action=None, **kwargs):
        """
        ...
        self.command_button = GlitchlessButton("Send", id=f"{self.id}-button", classes="command-value-button", action=action)

        # Create input fields with placeholders, each assigned a unique ID
        self.command_inputs: list[Input] = [
            Input("", f"{placeholders[i]}", id=to_valid_identifier(f"{placeholders[i]}-input"), classes="command-value-input")

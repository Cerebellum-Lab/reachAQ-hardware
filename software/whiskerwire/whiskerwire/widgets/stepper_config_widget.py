...
class StepperConfigWidget(Widget):
    ...
    def __init__(self, model: StepperModel, read_config: callable, write_config: callable, **kwargs):
        ...
        self.write_button = GlitchlessButton("Write", id=f"{self.id}-write-button", classes="command-value-button", action=self.write_stepper_config)
        
        # Button for reading the stepper config
        self.read_button = GlitchlessButton("Read", id=f"{self.id}-read-button", classes="command-value-button", action=self.read_stepper_config)
        
        # Create input fields with placeholders, each assigned a unique ID
        self.microsteps_widget = LabeledSelect(options=MICROSTEP_OPTIONS, prompt="Microsteps", id="microsteps-input", classes="command-value-select", allow_blank=True, label="Microsteps")
        
        self.steps_per_revolution_widget = LabeledInput(value="", placeholder="Steps per Revolution", id="steps-per-revolution-input", classes="command-value-input", label="Steps per Revolution")
    
        self.model._microsteps.register_on_update(self.on_microsteps_change)
        self.model._steps_per_revolution.register_on_update(self.on_steps_per_revolution_change)
    ...
    def write_stepper_config(self):
        ...
        if valid:
            self.write_config(min_step_inverse=microsteps, steps_per_revolution=steps_per_revolution)
    ...
    def compose(self):
        ...
        """
        yield Vertical(
                self.microsteps_widget, self.steps_per_revolution_widget, Horizontal(self.read_button, self.write_button, id="stepper-config-button-container"), classes="command-value-container"
            )

...
class MotorStatusWidget(StatusWidget):
    ...
    def __init__(self, model: MotorModel, move: callable, **kwargs):
        ...
        self.move = partial(
            move, motor_id=self.model.instance, abs_or_rel=AbsOrRel.ABSOLUTE
        )

        # Static displays for showing motor status and position
        self.status_display = Static(
            id="motor-status-display", classes="status-display"
        )
        self.position_display = Static(
            id="motor-position-display", classes="status-display"
        )

        # CommandValueWidget and JogButtons for controlling motor position
        self.command_position_widget = CommandValueWidget(
            "Command Position",
            ["position", "velocity", "acceleration"],
            action=self.command_position,
        )
        self.jog_buttons = JogButtons(
            f"{self.model.name} Jog Buttons",
            move=partial(move, motor_id=self.model.instance),
            small_jog_size=self.model.SMALL_JOG_SIZE,
            big_jog_size=self.model.BIG_JOG_SIZE,
            max_jog_velocity=self.model.JOG_VELOCITY,
            max_jog_acceleration=self.model.JOG_ACCELERATION,
        )
    ...
    def command_position(self):
        ...
        try:
            ...
            if self.model.is_valid_position(position):
                self.command_position_widget.set_input_error_status(
                    "position-input", False
                )
            else:
                self.command_position_widget.set_input_error_status(
                    "position-input", True
        ...
        try:
            ...
            if self.model.is_valid_acceleration(acceleration):
                self.command_position_widget.set_input_error_status(
                    "acceleration-input", False
                )
            else:
                self.command_position_widget.set_input_error_status(
                    "acceleration-input", True
                )
                valid = False
        except:
            self.command_position_widget.set_input_error_status(
        ...
        if valid:
            self.move(
                position=position, max_velocity=velocity, max_acceleration=acceleration
            )
    ...
    def on_mount(self):
        ...
        self.position_display.update(
            f"Position: {self.model.position:.3f}"

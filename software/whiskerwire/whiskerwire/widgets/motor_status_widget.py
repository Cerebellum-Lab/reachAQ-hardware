...
class MotorStatusWidget(StatusWidget):
    ...
    def __init__(self, model: MotorModel, move: callable, **kwargs):
        ...
        self.command_position_widget = CommandValueWidget("Command Position", ["position", "velocity", "acceleration"],
                                                          action=self.command_position)
        self.jog_buttons = JogButtons(f"{self.model.name} Jog Buttons",
                                      move=partial(move, motor_id=self.model.instance),
                                      small_jog_size=self.model.SMALL_JOG_SIZE, big_jog_size=self.model.BIG_JOG_SIZE,

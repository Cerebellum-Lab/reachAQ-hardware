...
class RGBLEDStatusWidget(StatusWidget):
    class InternalStatic(Static):
        def __init__(self, master: StatusWidget, renderable = "", *, expand = False, shrink = False, markup = True, name = None, id = None, classes = None, disabled = False):
    ...
    def __init__(self, model: RGBLEDModel, write: callable, **kwargs):
        ...
        self.color_display_widget = GlitchlessButton("", disabled=False, action=self.__toggle_selected, classes="color-display-widget")
        self.color_display_widget.active_effect_duration = 0.0001
        
        self.red_display_widget = self.InternalStatic(self, f"Red: {self.model.red}%", classes="rgb-status-display")
        self.green_display_widget = self.InternalStatic(self, f"Green: {self.model.green}%", classes="rgb-status-display")
        self.blue_display_widget = self.InternalStatic(self, f"Blue: {self.model.blue}%", classes="rgb-status-display")
        ...
        self.command_value_widget = CommandValueWidget("Set Color", ["Red", "Green", "Blue"], orientation=CommandValueWidget.Orientation.VERTICAL, action=self.command_rgb_led)
    ...
    def on_color_change(self):
        ...
        self.color_display_widget.set_styles(f"background: {rgb_to_hex(self.model.red*(255.0/100.0), self.model.green*(255.0/100.0), self.model.blue*(255.0/100.0))} 100%;")
    ...
    def compose_title(self):
        ...
        """
        return self.InternalStatic(self, f"[bold]{self.widget_name}[/bold]", classes="rgb-status-widget-title")

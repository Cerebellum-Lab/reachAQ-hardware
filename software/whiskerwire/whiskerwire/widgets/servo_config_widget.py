...
class ServoConfigWidget(Widget):
    ...
    def __init__(self, model: ServoModel, read_config: callable, write_config: callable, **kwargs):
        ...
        self.write_button = GlitchlessButton("Write", id=f"{self.id}-write-button", classes="command-value-button", action=self.write_servo_config)
        
        # Button for reading the servo config
        self.read_button = GlitchlessButton("Read", id=f"{self.id}-read-button", classes="command-value-button", action=self.read_servo_config)
        
        # Create input fields with placeholders, each assigned a unique ID
        self.min_position_widget = LabeledInput(value="", placeholder="Min Position", id="min-position-input", classes="command-value-input", label="Min Position")
        self.max_position_widget = LabeledInput(value="", placeholder="Max Position", id="max-position-input", classes="command-value-input", label="Max Position")
        self.min_pwm_duration_widget = LabeledInput(value="", placeholder="Min PWM Duration", id="min-pwm-duration-input", classes="command-value-input", label="Min PWM Duration")
        self.max_pwm_duration_widget = LabeledInput(value="", placeholder="Max PWM Duration", id="max-pwm-duration-input", classes="command-value-input", label="Max PWM Duration")
    ...
    def write_servo_config(self):
        ...
        if valid:
            self.write_config(min_position=min_position, max_position=max_position, min_pwm_duration_us=min_pwm_duration, max_pwm_duration_us=max_pwm_duration)

...
static void pressure_sensor_print_pressure_or_error(const struct shell *shell, const struct device *pressure_sensor,
                                                    int id) {
    const uint16_t pressure = ll_pressure_sensor_get_pressure(pressure_sensor);
    shell_print(shell, "Pressure #%d: %d", id, pressure);
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_pressure_sensor,
                               SHELL_CMD_ARG(read, NULL, "Read from a pressure sensor", cmd_pressure_sensor_read, 2, 0),
                               SHELL_CMD_ARG(dump, NULL, "Dump all readings from all pressure sensors",
                                             cmd_pressure_sensor_dump, 1, 0),
